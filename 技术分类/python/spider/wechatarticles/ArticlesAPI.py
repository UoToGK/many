...
class ArticlesAPI(object):
    ...
    def __init__(
        ...
        """
        # 两种登录方式, 扫码登录和手动输入登录
        if (token is not None) and (official_cookie is not None):
        ...
        if (appmsg_token is None) and (wechat_cookie is None) and (outfile is not None):
            from .ReadOutfile import Reader

            reader = Reader()
            reader.contral(outfile)
            self.appmsg_token, self.cookie = reader.request(outfile)
        elif (appmsg_token is not None) and (wechat_cookie is not None):
    ...
    def continue_info(self, nickname, begin=0):
        ...
        while True:
            try:
                # 获取文章数据
                artiacle_datas.append(self.officical.articles(nickname, begin=str(begin), count=str(count)))
            except Exception:
                break
            begin += count
            if begin > 40:
