...
class ArticlesUrls(object):
    ...
    def official_info(self, nickname, begin=0, count=5):
        ...
        try:
            # 返回与输入公众号名称最接近的公众号信息
            official = self.s.get(search_url, headers=self.headers, params=self.params)
    ...
    def query_articles_nums(self, nickname, query):
        ...
        try:
            return self.__get_articles_data(nickname=nickname, begin="0", query=query)[
                "app_msg_cnt"
            ]
        except Exception:
            raise Exception("公众号名称错误或cookie、token错误，请重新输入")
    ...
    def query_articles(self, nickname, query, begin, count=5):
        ...
        try:
            return self.__get_articles_data(
                nickname, begin=str(begin), count=str(count), query=query
            )["app_msg_list"]
        except Exception:
            raise Exception("公众号名称错误或cookie、token错误，请重新输入")

    def __get_articles_data(
        self, nickname, begin, count=5, type_="9", action="list_ex", query=None
    ):
        """
        获取公众号文章的一些信息
        Parameters
