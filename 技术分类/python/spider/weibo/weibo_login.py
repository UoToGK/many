"""
Author: xiaobin.zhu
since: 2022-03-08 20:20:58
LastAuthor: xiaobin.zhu
lastTime: 2022-05-16 15:37:37
Description: file content
FilePath: weibo_login.py
"""
...
def get_encrypted_mobile(token: str) -> str:
    params = {"token": token, "callback_url": "https://weibo.com"}
...
def send_code(token: str, encrypt_mobile: str) -> str:
    ...
    response = session.post(
        url=send_code_url, params=params, data=data, headers=headers
    ).json()
...
def confirm_code(encrypted_mobile: str, code: str, token: str) -> str:
    params = {"token": token}
    data = {"encrypt_mobile": encrypted_mobile, "code": code}
    response = session.post(
        url=confirm_url, params=params, data=data, headers=headers
    ).json()
    if response["msg"] == weibo_response_success_str:
