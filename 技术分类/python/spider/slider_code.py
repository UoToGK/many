...
Author: UoToGK
LastEditors: UoToGK
Date: 2022-04-17 21:25:48
LastEditTime: 2022-08-11 20:35:21
FilePath: slider_code.py
Description:  滑动验证码破解思路
...
class crack_picture(object):
    ...
    def pictures_recover(self):
        xpos = self.judge(
            self.picture_recover(self.img1, "img1.jpg"),
            self.picture_recover(self.img2, "img2.jpg"),
        )
        return self.darbra_track(xpos - 6)
...
class gsxt(object):
    ...
    def drag_pic(self):
        return (
            self.find_img_url(self.wait_for(By.CLASS_NAME, "gt_cut_fullbg_slice")),
    ...
    def find_img_url(self, element):
        try:
            return re.findall(r'url("(.*?)")', element.get_attribute("style"))[0].replace("webp", "jpg")
        except Exception:
            return re.findall(r"url((.*?))", element.get_attribute("style"))[0].replace("webp", "jpg")
    ...
    def emulate_track(self, tracks):
        ...
        for x, y, t in tracks:
            ActionChains(self.br).move_to_element_with_offset(to_element=element, xoffset=x + 22, yoffset=y + 22).perform()
        ...
        while flag:
            ...
            if "通过" in tsb:
                ...
                for sp in soup.find_all("a", attrs={"class": "search_list_item"}):
                    print(re.sub(r"\s+", "", sp.get_text().encode("utf-8")))
                break
            elif "吃" in tsb:
                time.sleep(5)
            else:
